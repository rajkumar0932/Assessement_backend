...
const AddProduct = async (req, res) => {
    try {
        ...
        if (!ProductID || !Name || Price === undefined || !Company) {
            return res.status(400).json({ message: "ProductID, Name, Price and Company are required" });
        }
...
const GetAllProducts = async (req, res) => {
    try {
        ...
        return res.status(200).json({ message: "All products", count: products.length, products });
    } catch (e) {
        return res.status(500).json({ message: e.message });
    }
};
...
const GetFeaturedProducts = async (req, res) => {
    try {
        ...
        return res.status(200).json({ message: "Featured products", count: products.length, products });
    } catch (e) {
        return res.status(500).json({ message: e.message });
    }
};
...
const GetProductsByPrice = async (req, res) => {
    try {
        ...
        if (Number.isNaN(value)) {
            return res.status(400).json({ message: "Provide a numeric ?value= for price" });
        }

        const products = await Product.find({ Price: { $lt: value } });
        return res.status(200).json({ message: `Products with price less than ${value}`, count: products.length, products });
    } catch (e) {
        return res.status(500).json({ message: e.message });
    }
};
...
const GetProductsByRating = async (req, res) => {
    try {
        const value = Number(req.query.value);
        if (Number.isNaN(value)) {
            return res.status(400).json({ message: "Provide a numeric ?value= for rating" });
        }

        const products = await Product.find({ Rating: { $gt: value } });
        return res.status(200).json({ message: `Products with rating higher than ${value}`, count: products.length, products });
    } catch (e) {
        return res.status(500).json({ message: e.message });
    }
};
